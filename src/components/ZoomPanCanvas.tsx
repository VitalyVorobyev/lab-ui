 ...
 * Transforming each layer separately would drift them apart at the exact zoom level where a
 * reader is trying to judge whether a caliper box lands on the edge it claims to measure.
 *
 ...
 * fitting is where you already are, and the thing a reader actually wants at a defect, or
 ...
 * what turns a cursor position into an index into a value plane or a source-pixel
 * coordinate, and it is wrong in a way nothing on screen would reveal — a readout that is
 * confidently off by a pan offset looks exactly like a readout that works.
 *
...
export function ZoomPanCanvas({
   ...
   */
  nativeWidth?: number;
  /** Hidden by passing `null` — a caller with its own fit button beside the canvas. */
  fitLabel?: ReactNode;
  /**
   ...
   * the one a value plane or a measurement overlay is indexed by however the frame is
   * zoomed or panned. Suppressed while dragging: during a pan the pointer is holding the
