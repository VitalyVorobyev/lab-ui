 ...
 *
 * Axis labels a reader can hold in their head are the whole reason: 0.25 / 0.50 / 0.75
 * rather than 0.237 / 0.474 / 0.711.
 */
 ...
 *
 * Derived from the step rather than fixed, because one chart here might be a loss around
 * 1e-4 and another a residual in whole pixels. Enough decimals to print the *step* exactly
 * is the right rule, and `ceil(-log10(step))` is not it: a 0.25 step gets one decimal that
 ...
 * which is what watching the first moments of a training run, or a profile with one sample
 * so far, should look like.
 */
...
/**
 * A base-10 log scale, for series that span decades.
 *
 * A loss curve is the canonical case: three terms that span decades and start far apart
 * are a flat line along the bottom for two of the three on a linear axis. Non-positive
 * values are clamped to `LOG_FLOOR` rather than dropped: a value that reaches exactly zero
 * is a real observation, and a gap in the line would read as missing data.
 */
