/**
 * An options form, generated from a JSON Schema.
 *
 * Adding an adapter, a model, or a measurement method costs a config module and a registry
 * entry, and *nothing here* — that is the point of generating the form from the schema
 * instead of hardcoding it. Every decision about which control a field deserves lives in
 * `api/schemaForm.ts`; this file only renders.
 *
 * Two behaviours are worth knowing while reading the markup:
 *
 *   * An empty control means **unset**, and the caller's own default applies. The default
 *     is shown — as a placeholder, as the pre-highlighted segment, or as the picker's
 *     leading entry — so a reader can see what happens if they say nothing, without that
 *     value being claimed as their choice.
 *   * A field the schema requires is marked, and the caller blocks the run on it.
 *
 * The layout is a two-column grid: a form of eight numeric parameters, each a full-width
 * control over a full-width description, runs to screens of scrolling for a form that
 * often needs nothing touched at all. Fields whose value is genuinely wide — a list, a
 * JSON object, a statement with a switch — still span both columns.
 */
 ...
 * in a row can run as tall as the layout this rule exists to avoid.
 */
...
export function SchemaForm({
  ...
  // Where nothing is required — the normal shape of a model's own hyperparameters, as
  // against an adapter's "which folder holds the good images" — there is nothing to fold
  // away from. Hiding every field behind a disclosure would leave a box containing only a
