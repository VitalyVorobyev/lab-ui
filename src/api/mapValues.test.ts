...
describe("decodePlane", () => {
  ...
  it("takes the channel count from the header rather than assuming it", () => {
    /**
     * A mono plane has one channel and an RGB one has three. Nothing here may encode
     * which — channel count is data, so the header carries it and both are the same code
     * path.
     */
    const plane = decodePlane(
      encode([
...
describe("valueAt", () => {
  ...
  it("indexes by fractions of the frame", () => {
    // No letterbox correction: the plane is assumed to cover the frame edge to edge.
