...
/** The shapes pydantic actually emits, copied from a live options schema. */
...
describe("describeFields", () => {
  ...
  it("shows the default as a placeholder", () => {
...
describe("what an operator has to decide", () => {
  it("keeps a field with no useful default in front of them", () => {
    // `filter_column` defaults to null: nothing can filter until you say on what.
  ...
  it("folds away a field that already has an answer", () => {
  ...
  it("treats an empty list default as something still to supply", () => {
    // This is the rule that puts a required-in-spirit field at the top of the form.
...
describe("toOptions", () => {
  it("leaves out anything untouched, so the default stays the only one", () => {
...
describe("a schema with no options at all", () => {
 ...
 *
 * These are the exact shapes pydantic emits for a `Literal` and a `StrEnum`, and every one
 * of them reaches the wrong control if `type` is tested before `enum`, or if `$ref` is not
...
describe("a closed set of values", () => {
  ...
  it("labels the field, not the enum class it points at", () => {
    ...
    // the name of a Python type nobody filling in the form has heard of.
  ...
  it("still falls back to a JSON textarea for a $ref that is not a closed set", () => {
    // The escape hatch has to survive: a caller can express what is accepted even where
    // the form has nothing prettier to offer.
...
describe("sending a choice", () => {
  it("sends nothing for a choice nobody touched", () => {
    ...
    // the default stays defined in exactly one place.
...
/**
 * The bounds were in the schema the whole time. Dropping them means an out-of-range value
 * is reported as a rejection from wherever it lands, half a screen away from the field that
 * caused it.
 */
